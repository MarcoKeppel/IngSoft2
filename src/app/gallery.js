...
router.get("", async (req, res) => {
	...
	let users = await User.find().select('email posts -_id').populate('posts', 'time path -_id').lean();
...
router.get("/me", tokenChecker, async (req, res) => {
	...
	let user = await User.findOne({_id: req.loggedUser.id}).select('email posts -_id').populate('posts', 'time path -_id').lean();
...
router.get("/:username", async (req, res) => {
	...
	// Get all users from DB
	let user = await User.findOne({username: req.params.username}).select('email posts -_id').populate('posts', 'time path -_id').lean();
